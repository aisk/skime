...
INSTRUCTION_SET_PATH = PACKAGE_DIR / "iset.yml"
GENERATED_PATHS = {
    "iset": PACKAGE_DIR / "iset.py",
    "insns": PACKAGE_DIR / "insns.py",
}
...
def gen_actions(instructions):
    def gen_action(insn):
        ...
        env = {"insn_len": 1 + len(insn["operands"])}
        code = insn["code"]
    ...
    return "\n".join([gen_action(insn) for insn in instructions])
...
def gen_tags_table(instructions):
    def gen_tag(insn):
        if len(insn["tags"]) == 0:
            ...
            return " | ".join(["TAG_%s" % tag.upper() for tag in insn["tags"]])
...
def gen_insn_table(instructions):
    ...
    return (
        ...
        + ",\n".join(
            [
                "    " + insn["name"].__repr__() + " : INSTRUCTIONS[%d]" % i
                for i, insn in insns
            ]
        )
        + "\n}\n"
...
TMPL_INSNS = """\
# Don't edit this file. This is generated by python -m skime.iset_gen.

from .call_cc import Continuation
from .invoke import CallFrame, invoke
...
def make_call(ctx, argc, tail=False):
    proc = ctx.pop()
    args = [ctx.top(idx=argc-i) for i in range(argc)]
    ctx.pop_n(argc)
    ...
    result = invoke(proc, args, parent, ctx.vm)
...
def render_outputs():
    iset = yaml.safe_load(INSTRUCTION_SET_PATH.read_text(encoding="utf-8"))
    env = {
    ...
    return {
        "iset": black.format_str(process_tmpl(TMPL_ISET, env), mode=mode),
        "insns": black.format_str(process_tmpl(TMPL_INSNS, env), mode=mode),
    }


def main(argv=None):
    parser = argparse.ArgumentParser(description="Generate VM instruction modules")
    parser.add_argument(
        "--check",
        action="store_true",
        help="fail if generated modules are not up to date",
    )
    ...
    if stale:
        parser.error("generated files are out of date: %s" % ", ".join(stale))
